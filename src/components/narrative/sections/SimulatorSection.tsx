...
export function SimulatorSection() {
  ...
  const taxableWealth = taxBase === 'millionaires' 
    ? wealth.millionaire_population.count_individuals * 2500000
  ...
  return (
    <NarrativeSection id="simulator" ariaLabel="Policy Simulator" className="py-16 md:py-24 bg-muted/30">
      <div className="container-wide">
        ...
        {/* Warning Banner */}
        <motion.div 
          className="glass-card p-4 border-l-4 border-destructive mb-8"
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              <strong className="text-foreground">{t('simulator.disclaimer.title')}</strong> {t('simulator.disclaimer.text')}
            </p>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <SourceDrawer sourceIds={poverty.official_key_statistics.source_ids} title={t('simulator.baseline.dataTitle')} />
          </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex gap-2">
                  {(['millionaires', 'billionaires'] as const).map(opt => (
                    <button 
                      key={opt} 
                      onClick={() => setTaxBase(opt)} 
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                        taxBase === opt ? 'bg-secondary text-primary' : 'bg-muted hover:bg-muted/80'
                      }`}
                    >
                      {t(`simulator.controls.${opt}`)}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="border-t pt-4">
                ...
                <div className="space-y-3">
                  <div>
                    <span className="text-sm">{t('simulator.controls.povertyGap', { percent: povertyAlloc })}</span>
                    <Slider value={[povertyAlloc]} onValueChange={([v]) => setPovertyAlloc(Math.min(v, 100 - servicesAlloc))} min={0} max={100} className="mt-1" />
                  </div>
                  <div>
                    <span className="text-sm">{t('simulator.controls.services', { percent: servicesAlloc })}</span>
                    <Slider value={[servicesAlloc]} onValueChange={([v]) => setServicesAlloc(Math.min(v, 100 - povertyAlloc))} min={0} max={100} className="mt-1" />
                  </div>
